...
//--------------------Rotating Calipus--------------------
double rotatingCalipus(vector<pii> &convex) {
    ...
    for(int k = 0; k <= sz; k++) {
        double d = dist(convex[i], convex[j]);
        ret = max(ret, d);
...
int main()
{
    ...
    if(convexHull.size() == 1) cout << 0;
        ...
        cout << setprecision(12) << dist(convexHull[0], convexHull[1]);
    else
        cout << setprecision(12) << rotatingCalipus(convexHull);
